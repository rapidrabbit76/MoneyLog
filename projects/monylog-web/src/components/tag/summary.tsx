...
export function TagSummary({ expenses }: TagSummaryProps) {
  ...
  const tagData = useMemo(() => {
    ...
    const totalExpense = Array.from(tagMap.values()).reduce(
      (sum, cat) => sum + cat.expenseAmount,
      0,
    );
    const totalIncome = Array.from(tagMap.values()).reduce(
      (sum, cat) => sum + cat.incomeAmount,
      0,
    );
    ...
    const result: TagData[] = Array.from(tagMap.values()).map((cat) => ({
      ...cat,
      expensePercentage:
        totalExpense > 0 ? (cat.expenseAmount / totalExpense) * 100 : 0,
      incomePercentage:
        totalIncome > 0 ? (cat.incomeAmount / totalIncome) * 100 : 0,
    }));
        ...
        <CardContent className="space-y-2">
          ...
          {viewType === "both" && (
            <div className="flex justify-between items-center pt-2 border-t">
              ...
              <div
                className={`text-sm font-bold ${getNetAmountColor(totals.net)}`}
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {viewType === "both" && (
                      <div className="flex items-center gap-1">
                        ...
                        <span
                          className={`text-xs font-medium ${getNetAmountColor(cat.netAmount)}`}
                        >
                  ...
                  {viewType !== "both" && (
                    <div className="text-xs text-muted-foreground text-center">
                      전체의{" "}
                      {viewType === "expense"
                        ? cat.expensePercentage.toFixed(1)
                        : cat.incomePercentage.toFixed(1)}
                      %
                    </div>
                  )}
                </div>
